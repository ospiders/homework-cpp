...
public:
	...
	const Line operator= (const Line& object) { _line = object._line; return *this; }
	...
	const Line operator+ (const Line& object) const { return Line{ _line + object._line }; }
	const Line operator+ (std::string line) const { return Line{ _line + line }; }

	const Line operator+= (const Line& object) { return Line{ _line += object._line }; }
...
public:
	...
	BitLine& operator= (const BitLine& object) { this->set_Line(object.get_Line()); return *this; }
	BitLine& operator= (std::string line) { this->set_Line(line); return *this; }
	...
	const BitLine operator+ (const BitLine object)
	{
		this->to_TwosCode();
		object.to_TwosCode();
		...
		while (size1 >= 0 || size2 >= 0 || temp == 1) {
			...
			temp /= 2; // если 2 единицы в темп, то  2 / 2 = 1 (1 + 1 = 0) + 1  в следующий разряд
			size1--; size2--;
		}
	...
	const BitLine operator+= (const BitLine& object) { *this = BitLine{ *this + BitLine{object} }; return *this; }
	const BitLine operator+= (std::string line) { return *this += BitLine{line}; }

    friend bool operator==(BitLine& l_object, BitLine& r_object) { return (l_object.get_Line() == r_object.get_Line()); }
    friend bool operator==(BitLine& l_object, std::string line) { return (l_object.get_Line() == line); }
